...
// Both goldens are computed live into test/generated by the oracle project's
// global setup, so the calibration always measures the installed oracles.
const fixtureUrl = new URL('./generated/A/sphere/', import.meta.url);
...
async function loadMesh(filename: string) {
  return readMeshJson(await readFile(new URL(filename, fixtureUrl), 'utf8'));
}
