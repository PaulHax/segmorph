...
describe('contour plane', () => {
  ...
  it('rejects invalid origins and bases', () => {
    expect(() => createContourPlane([0, 0, Number.NaN], [1, 0, 0], [0, 1, 0])).toThrow(/origin/);
    expect(() => createContourPlane([0, 0, 0], [2, 0, 0], [0, 1, 0])).toThrow(/orthonormal/);
    expect(() => createContourPlane([0, 0, 0], [1, 0, 0], [1, 0, 0])).toThrow(/orthonormal/);
  });
...
describe('contour loops', () => {
  ...
  it('rejects incomplete, non-finite, and undersized loops', () => {
    expect(() => validateContourLoop({ points: new Float64Array([0, 0, 1]) })).toThrow(
      /complete xy/,
    );
    expect(() =>
      createContourLoop([
...
describe('planar contour', () => {
  it('constructs and validates a plane with closed loops', () => {
    ...
    const first = createContourLoop([
      [0, 0],
      [2, 0],
      [0, 2],
    ]);
    const second = createContourLoop([
      [3, 3],
      [4, 3],
      [3, 4],
    ]);
